'''
This file is the client file for player 4. By executing this file you can play
    ...
    def timerFired(self, dt):
        ...
        if self.mode == 'shoot':
            ...
            if self.isKeyPressed(pygame.K_a):
                self.player4.lastDir = 'left'
                self.player4.x -= self.player4.speed
                self.player4.update(self.isKeyPressed)
                wall = pygame.sprite.spritecollideany(self.player4, self.wallGroup)
                if wall != None:
                    playerLeftEdge = self.player4.x - self.player4.width/2
                    wallRightEdge = wall.x + wall.width/2
                    overlap = wallRightEdge - playerLeftEdge
                    self.player4.x += overlap
                    self.player4.update(self.isKeyPressed)
                msgList.append('move %d %d %s\n' % 
                    (self.player4.x, self.player4.y, self.player4.lastDir))
                    
            if self.isKeyPressed(pygame.K_d):
                self.player4.lastDir = 'right'
                self.player4.x += self.player4.speed
                self.player4.update(self.isKeyPressed)
                wall = pygame.sprite.spritecollideany(self.player4, self.wallGroup)
                if wall != None:
                    playerRightEdge = self.player4.x + self.player4.width/2
            ...
            if self.isKeyPressed(pygame.K_w):
                self.player4.lastDir = 'up'
                self.player4.y -= self.player4.speed
                self.player4.update(self.isKeyPressed)
                wall = pygame.sprite.spritecollideany(self.player4, self.wallGroup)
                if wall != None:
                    playerTopEdge = self.player4.y - self.player4.height/2
                    wallBotEdge = wall.y + wall.height/2
                    overlap = wallBotEdge - playerTopEdge
                    self.player4.y += overlap
                    self.player4.update(self.isKeyPressed)
                if self.mode == 'shoot':
                    msgList.append('move %d %d %s\n' % 
                        (self.player4.x, self.player4.y, self.player4.lastDir))
            ...
            if self.isKeyPressed(pygame.K_s):
                self.player4.lastDir = 'down'
                self.player4.y += self.player4.speed
                self.player4.update(self.isKeyPressed)
                wall = pygame.sprite.spritecollideany(self.player4, self.wallGroup)
                if wall != None:
                    playerBotEdge = self.player4.y + self.player4.height/2
