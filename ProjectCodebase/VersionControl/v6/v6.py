...
'''
1. find route according to directions (DONE)
2. convert from directions to set of coordinates (in terms of absolute x and y values
    to feed the AI) (DONE)
3. move the ai accordingly to get to those coordinates by finding the difference
    between the abs x and y values
'''
...
class ComputerChase(GameCharacter):
    ...
    def newRoute(self):
        self.targetPos = ketchUp.player1.pos
    ...
    def move(self):
        ...
        # if diffX > 0: # to the right
        #     if abs(diffX) >= 0.1*ketchUp.wallSize:
        #         self.x += self.speed
        # elif diffX < 0: # to the left
        #     if abs(diffX) >= 0.1*ketchUp.wallSize:
        #         self.x -= self.speed
        # if diffY > 0: # downwards
        #     if abs(diffY) >= 0.1*ketchUp.wallSize:
        #         self.y += self.speed
        # elif diffY < 0: # upwards
        #     if abs(diffY) >= 0.1*ketchUp.wallSize:
        #         self.y -= self.speed
