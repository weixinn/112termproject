...
# from last try, added chase and run AI and bg music and shield
# things to do: - clean up code
#               - fix running AI
#               - sockets
...
what the running AI does every second:
    1. finds the chaser
    2. sets target position to a space that is exactly the board length num of
       steps away
    3. tries to find a route to that position that ensures it doesn't go more than
       a number of steps within range of the chaser
...
class Computer(GameCharacter):
    ...
    def newRoute(self):
        if self.mode == 'chase':
            ...
            '''
            if have time, change the function to look at all possible target positions
            
            # seenTargetPos = set()
            # 
            # while routeDir != None:
            #     self.targetPos = findPosNumStepsAway(ketchUp.wallMap, chaserPos, 
            #                       self.numRunSteps, seenTargetPos)
            '''
...
class myProject(pygamegame.PygameGame):
    ...
    def initNewGame(self):
        ...
        testingabc = (str(self.wallMap))
        if ' ' in testingabc:
            print (True)
        else:
            print (False)
        testingabc = removeSpacesForBoard(testingabc)
        if ' ' in testingabc:
            print (True)
        else:
            print (False)
    ...
    def drawScoreChase(self, screen):
        ...
        # player 3
        name3 = self.standardFont.render("Mustard", True, (255,255,255))
        ...
        # player 4
        name4 = self.standardFont.render("Mustard", True, (255,255,255))
