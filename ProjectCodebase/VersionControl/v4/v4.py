...
# from last try, added variable board sizes, added items: gun, snail, boot, laser
# things to do:
#               - add in shield
#               - bg music
#               - basic AI
#               - clean up code
#               - user interface e.g. start screen, etc
...
class ComputerChase(GameCharacter):
    def move(self, targetPos, speed):
        route = findRoute(ketchUp.wallMap, self.pos, targetPos)
        move = route[0]
        dCol, dRow = move
        dX = dCol*speed
        dY = dRow*speed
...
class myProject(pygamegame.PygameGame):
    ...
    def timerFired(self, dt):
        if self.mode == 'chase' or self.mode == 'shoot':
            ...
            self.player3.move((0,9), self.speed)
        
            self.time += 1
            
            # this whole chunk is only temporary
            if self.time%50 == 0:
                self.count += 1
                if self.count > 10:
                    self.count = 0
    ...
    def drawScore(self, screen):
        ...
        timeLeft = self.boldFont.render("Time left: %d" %self.timeLeft, True, (255,255,255))
