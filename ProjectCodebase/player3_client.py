'''
This file is the client file for player 3. By executing this file you can play
...
HOST = "128.237.166.12" # put server IP address here if playing on multiple computers
PORT = 50003
    ...
    def timerFired(self, dt):
        ...
        if self.mode == 'shoot':
            ...
            if self.isKeyPressed(pygame.K_a):
                self.player3.lastDir = 'left'
                self.player3.x -= self.player3.speed
                self.player3.update(self.isKeyPressed)
                wall = pygame.sprite.spritecollideany(self.player3, self.wallGroup)
                if wall != None:
                    playerLeftEdge = self.player3.x - self.player3.width/2
                    wallRightEdge = wall.x + wall.width/2
                    overlap = wallRightEdge - playerLeftEdge
                    self.player3.x += overlap
                    self.player3.update(self.isKeyPressed)
                msgList.append('move %d %d %s\n' % 
                    (self.player3.x, self.player3.y, self.player3.lastDir))
                    
            if self.isKeyPressed(pygame.K_d):
                self.player3.lastDir = 'right'
                self.player3.x += self.player3.speed
                self.player3.update(self.isKeyPressed)
                wall = pygame.sprite.spritecollideany(self.player3, self.wallGroup)
                if wall != None:
                    playerRightEdge = self.player3.x + self.player3.width/2
            ...
            if self.isKeyPressed(pygame.K_w):
                self.player3.lastDir = 'up'
                self.player3.y -= self.player3.speed
                self.player3.update(self.isKeyPressed)
                wall = pygame.sprite.spritecollideany(self.player3, self.wallGroup)
                if wall != None:
                    playerTopEdge = self.player3.y - self.player3.height/2
                    wallBotEdge = wall.y + wall.height/2
                    overlap = wallBotEdge - playerTopEdge
                    self.player3.y += overlap
                    self.player3.update(self.isKeyPressed)
                if self.mode == 'shoot':
                    msgList.append('move %d %d %s\n' % 
                        (self.player3.x, self.player3.y, self.player3.lastDir))
            ...
            if self.isKeyPressed(pygame.K_s):
                self.player3.lastDir = 'down'
                self.player3.y += self.player3.speed
                self.player3.update(self.isKeyPressed)
                wall = pygame.sprite.spritecollideany(self.player3, self.wallGroup)
                if wall != None:
                    playerBotEdge = self.player3.y + self.player3.height/2
