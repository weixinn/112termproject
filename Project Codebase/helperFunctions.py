'''
This file contains all the helper functions that is needed for the game to function

Some important ones to note:
    1. generateBoard
    2. findRoute (for easy AI)
    3. aStarFindRoute (for hard chasing AI)
    4. forRunAStarFindRoute (for hard running AI)
    
Reference:
I wrote the aStar functions after understanding the concepts from:
...
# concept taken from: https://en.wikipedia.org/wiki/Pathfinding
...
# for me to read and debug more easily
...
def convertPowerUpSpritesToStr(spriteGroup):
    string = ''
    
    for powerUp in spriteGroup:
        row = str(powerUp.row) + '#'
        col = str(powerUp.col) + '#'
        powerUpType = str(type(powerUp)) + '#'
        width = str(powerUp.width) + '#'
        height = str(powerUp.height) + '#'
        string = string + row + col + powerUpType + width + height + '&'
